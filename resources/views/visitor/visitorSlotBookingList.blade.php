...
<div class="container">
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="card-title">Event Slot Booking Request List</h4>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th>{{ ($slotBooking->currentPage() - 1) * $slotBooking->perPage() + $loop->index + 1 }}
                            </th>
                            <td>{{ $slotBookingData->slots->start_time ?? '' }} - {{ $slotBookingData->slots->end_time
                                ?? '' }}</td>
                            <td>{{ $slotBookingData->users->firstName ?? '' }} {{ $slotBookingData->users->lastName ??
                                '' }}</td>
                            {{-- <td>{{ $slotBookingData->visitors->firstName ?? '' }} {{
                                $slotBookingData->visitors->lastName ?? '' }}</td> --}}
                            {{-- <td>{{ $slotBookingData->refMembers->firstName ?? '' }} {{
                                $slotBookingData->refMembers->lastName ?? '' }}</td> --}}
                            <td>{{ \Carbon\Carbon::parse($slotBookingData->date)->format('d-m-Y') ?? '' }}</td>
                            <td>
                                <form action="{{ route('slotBooking.updateStatus', $slotBookingData->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="card-title">Event Slot Booking Requested List</h4>
                {{-- <a href="{{ route('event.index') }}" class="btn btn-bg-orange btn-sm">BACK</a> --}}
            </div>
