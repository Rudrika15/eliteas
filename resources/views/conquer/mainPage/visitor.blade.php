            ...
            <div class="row d-flex justify-content-center">
                <div class="col-xl-7 col-lg-8 col-md-9 col-11 text-center">
                    ...
                    <div class="card p-3" style="border-radius: 10px; background-color: rgba(255, 255, 255, 0.699);">
                        ...
                        <form method="POST" action="{{ route('conquer.visitor.form.store') }}" class="needs-validation w-100 form-card" id="visitorForm" name="visitorForm" novalidate>
                            @csrf
                            ...
                            <div class="row justify-content-between text-left">
                                <div class="form-group col-sm-6 flex-column d-flex">
                                    ...
                                    <input type="text" id="mobileNo" name="mobileNo" value="{{ old('mobileNo') }}" class="form-control @error('mobileNo') is-invalid @enderror" oninput="if(this.value.length > 10) this.value = this.value.slice(0,10); this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" required>
                                    @error('mobileNo')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="row justify-content-between text-left">
                                <div class="form-group col-sm-6 flex-column d-flex">
                                    ...
                                    <input type="date" id="birthDate" name="birthDate" class="form-control @error('birthDate') is-invalid @enderror" required>
                                    @error('birthDate')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button" id="proceedWithCoupon" class="btn btn-success d-none">Pay with
                                                Discount</button>
                                            <button type="button" id="proceedWithoutCoupon" class="btn btn-secondary">Pay Original
    ...
    Total Amount is: ₹${originalAmount / 100} <br>  Coupon applied successfully! You get a discount of
        ₹${data.discount}. Total payable amount: ₹${discountedAmount / 100}.</div>`;
                ...
                proceedWithCouponBtn.addEventListener('click', function() {
                    console.log('Proceed with coupon button clicked');
                    couponModal.hide();
                    proceedToPayment(discountedAmount);
                });

                proceedWithoutCouponBtn.addEventListener('click', function() {
                    console.log('Proceed without coupon button clicked');
                    couponModal.hide();
            ...
            function proceedToPayment(amount) {
                console.log('Proceeding to payment:', amount);
                // const razorpayKey = "{{ env('RAZORPAY_KEY') }}";
                const razorpayKey = "rzp_test_VVNmvqg0nEoaOf";
            ...
            function storePaymentDetails(paymentId, amount, formData) {
                console.log('Storing payment details:', paymentId, amount, formData);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            console.error('Error storing payment details:', data);
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Payment succeeded but failed to store details.'
                            });
                        }
                    })
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error storing payment details.'
                        });
                    });
            }
        });
