@forelse ($sentRequests ?? [] as $member)
    <div class="col-12 col-sm-6 col-lg-6">
        <div class="profile-card shadow-sm rounded border-0">
            ...
            <div class="text-center p-3">
                <img src="{{ asset('ProfilePhoto/' . ($member->receiver->profilePhoto ?? 'profile.png')) }}" class="profile-img img-fluid rounded-circle mx-auto d-block" alt="Profile Image">
                <h5 class="member-name" style="color: #1d3268; font-weight: bold;">
                    {{ $member->receiver->firstName ?? 'N/A' }} {{ $member->receiver->lastName ?? 'N/A' }}
                </h5>
                ...
                <div class="info-section">
                    {{-- @php
                                    ...
                                    @endphp --}}
                    <div class="icon-text" title="{{ $member->receiver->email ?? 'N/A' }}">
                        ...
                        <span>{{ Str::limit($member->receiver->email ?? 'N/A', 15) }}</span>
                    </div>
                    <div class="icon-text" title="{{ $member->receiver->contactNo ?? 'N/A' }}">
                        {{-- <i class="bi bi-telephone-fill text-muted"></i> --}}
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#787C80">
                            <path d="M798-120q-125 0-247-54.5T329-329Q229-429 174.5-551T120-798q0-18 12-30t30-12h162q14 0 25 9.5t13 22.5l26 140q2 16-1 27t-11 19l-97 98q20 37 47.5 71.5T387-386q31 31 65 57.5t72 48.5l94-94q9-9 23.5-13.5T670-390l138 28q14 4 23 14.5t9 23.5v162q0 18-12 30t-30 12ZM241-600l66-66-17-94h-89q5 41 14 81t26 79Zm358 358q39 17 79.5 27t81.5 13v-88l-94-19-67 67ZM241-600Zm358 358Z" />
                        </svg>
                        <span>{{ Str::limit($member->receiver->contactNo ?? 'N/A', 10) }}</span>
                    </div>
                    <div class="icon-text">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#787C80">
                        ...
                        <span>{{ $member->receiverMember->circle->circleName }}</span>
                    </div>
                </div>
                <div class="company-category-section">
                    <div class="company-section">
                        <div class="logo-section">
                            @if (!empty($member->receiverMember->companyLogo))
                                <img src="{{ asset('CompanyLogo/' . $member->receiverMember->companyLogo) }}" class="company-logo" alt="Company Logo">
                            @endif
                            <div class="initials" style="{{ empty($member->receiverMember->companyLogo) ? 'display:flex;' : 'display:none;' }}">
                                {{ strtoupper(substr($member->receiverMember->companyName ?? 'C', 0, 1)) }}
                            </div>
                        </div>
                        <h2 title="{{ $member->receiverMember->companyName ?? 'Company Name' }}">
                            {{ $member->receiverMember->companyName ?? 'Company Name' }}
                        </h2>
                    </div>
                    <div class="divider"></div>
                    <div class="category-section">
                        <div class="label">Category</div>
                        <h3>{{ $member->receiverMember->bCategory->categoryName ?? 'N/A' }}</h3>
                    </div>
                </div>
                <div class="keywords-container row">
                    @php
                        $keyWords = json_decode($member->receiverMember->keyWords ?? '[]', true);
                    @endphp
            ...
            <div class="bottom-actions">
                <div id="viewProfile" class="action-button left-action">
                    <a href="{{ route('foundPersonDetails', $member->receiverMember->id) }}">
