        ...
        <div class="col">
            <div class="fb-card shadow-sm h-100">
                ...
                <div class="fb-card-body">
                    ...
                    @if (!empty($member->receiverMember->companyName) || !empty($member->receiverMember->bCategory->categoryName))
                        <div class="fb-card-info">
                            @if (!empty($member->receiverMember->companyName))
                                <div><i class="bi bi-building"></i> {{ $member->receiverMember->companyName }}</div>
                            @endif
                            @if (!empty($member->receiverMember->bCategory->categoryName))
                                <div><i class="bi bi-tag"></i> {{ $member->receiverMember->bCategory->categoryName }}</div>
                            @endif
                        </div>
                    @endif
                    @php $keyWords = json_decode($member->receiverMember->keyWords ?? '[]', true); @endphp
