        ...
        <div class="col">
            <div class="fb-card shadow-sm h-100">
                ...
                <div class="fb-card-body">
                    <h5 class="fb-card-title">{{ $member->user->firstName ?? 'N/A' }} {{ $member->user->lastName ?? 'N/A' }}</h5>
                    ...
                    @if (!empty($member->members->companyName) || !empty($member->members->bCategory->categoryName))
                        <div class="fb-card-info">
                            @if (!empty($member->members->companyName))
                                <div><i class="bi bi-building"></i> {{ $member->members->companyName }}</div>
                            @endif
                            @if (!empty($member->members->bCategory->categoryName))
                                <div><i class="bi bi-tag"></i> {{ $member->members->bCategory->categoryName }}</div>
                            @endif
                        </div>
                    @endif
                    @php $keyWords = json_decode($member->members->keyWords ?? '[]', true); @endphp
                    ...
                    <div class="mt-auto">
                        ...
                        <div class="d-flex gap-2 mt-2">
                            ...
                            <a href="{{ route('connection.accept', $member->id) }}" class="fb-btn fb-btn-secondary flex-fill"><i class="bi bi-check-circle me-2"></i> Accept</a>
                        </div>
                    </div>
                </div>
            </div>
