...
<style>
    .birthday-banner {
        ...
        border-radius: 22px;
        padding: 35px 45px;
        background: linear-gradient(135deg, #0D47A1 0%, #1565C0 45%, #FF8F00 100%);
        color: #fff;
...
<div class="birthday-banner mb-4">
    ...
    <div class="birthday-content">
        ...
        <div class="profile-wrapper">
            ...
            <img src="{{ asset('ProfilePhoto/' . $member->profilePhoto) }}" alt="{{ $member->firstName }}">
            @else
        ...
        <div class="birthday-info">
            ...
            <span class="badge-birthday">
                🎂 TODAY'S CELEBRATION
            </span>

            <h2>
                Happy Birthday, {{ $member->firstName }} {{ $member->lastName }}! 🎉
            </h2>

            <p>
                Wishing you a wonderful year filled with happiness,
                success, prosperity and unforgettable memories.
            </p>
            ...
            <div class="col-sm-6 col-lg-3">
                <div class="rising-star-card">
                    <div class="rising-star-img-wrapper">
                        ...
                        <img src="{{ asset('ProfilePhoto/' . ($star->member->profilePhoto ?? 'profile.png')) }}" class="rising-star-img" alt="Profile Image">
                    </div>
                    <div class="rising-star-body">
                        ...
                        <div class="mt-auto">
                            @if(auth()->check() && (auth()->user()->member?->id ?? $member->id ?? null) != $star->member_id)
                            @php
                            $connectionStatus = $star->member->connection_status ?? 'Not Connected';
                            @endphp
                            @if ($connectionStatus == 'Connected')
                            <button type="button" class="fb-btn fb-btn-disabled">
                                Connected
                            </button>
                            @elseif($connectionStatus == 'Accepted')
                            ...
                            @elseif($connectionStatus == 'Pending' || $connectionStatus == 'Requested')
                            <button type="button" class="fb-btn fb-btn-disabled">
                                Requested
                            </button>
                            @else
