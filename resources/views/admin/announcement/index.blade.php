        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <a href="{{ route('announcement.create') }}" class="btn btn-bg-orange btn-sm mt-3 btn-tooltip"><i class="bi bi-plus-circle"></i>
                    ...
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($announcements as $announcementData)
                                <tr>
                                    <th>{{ ($announcements->currentPage() - 1) * $announcements->perPage() + $loop->index + 1 }}

                                    <td>{{ $announcementData->title ?? '-' }}</td>
                                    <td>
                                        {{ $announcementData->description ?? '-' }}
                                    </td>
                                    <td>{{ $announcementData->start_date ?? '-' }}</td>
                                    <td>{{ $announcementData->end_date ?? '-' }}</td>
                                    <td>{{ $announcementData->status }}</td>
                                    <td>

                                        <a href="{{ route('announcement.edit', $announcementData->id) }}" class="btn btn-bg-blue btn-sm btn-tooltip">
                                            <i class="bi bi-pen"></i>
                                            <span class="btn-text">Edit</span>
                                        </a>

                                        <a href="{{ route('announcement.delete', $announcementData->id) }}" class="btn btn-danger btn-sm btn-tooltip">
