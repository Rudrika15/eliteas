            ...
            <div class="card-body d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex gap-2">
                    <form action="{{ route('attendance.lockMeeting', $meetingId) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-sm {{ $currentSchedule->is_locked ? 'btn-danger' : 'btn-success' }}">
                                ...
                                <tr>
                                    ...
                                    @php
                                        $stats = $memberStats[$member->userId] ?? [
                                            ...
                                            'ref_given_inside' => 0,
                                            'ref_given_outside' => 0,
                                            'ref_received_inside' => 0,
                                            'ref_received_outside' => 0,
                                            'business_given' => 0,
                                            'business_received' => 0,
                                            'training' => 0,
                                            'testimonial_given' => 0,
                                            'testimonial_received' => 0,
                                            'present' => 0,
                                            'absent' => 0,
                                            'late' => 0,
                                            'medical' => 0,
    ...
    <script>
        $(function() {
            ...
            $(document).on('change', '.attendance-select', function() {
                const $select = $(this);
                // Just update the color class locally, no AJAX call
                $select
                    .removeClass('present absent late medical sub none')
                    .addClass($select.val().toLowerCase() || 'none');
            });
