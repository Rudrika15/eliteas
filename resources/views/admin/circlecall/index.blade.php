        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover mb-4">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $circlecallData->meetingPerson->firstName ?? '-' }} {{ $circlecallData->meetingPerson->lastName ?? '-' }}</td>
                        <td>{{ $circlecallData->meetingPlace ?? '-' }}</td>
                        <td>
                            @if ($circlecallData->meetingImage)
                            <img src="{{ url('meetingImage/' . basename($circlecallData->meetingImage)) }}" alt="Meeting Image" style="width: 100px; height: auto; border-radius: 5px;">
                        @else
                            <span></span>
                        @endif
                        </td>
                        ...
                        <td>
                            <a href="{{ route('circlecall.edit', $circlecallData->id) }}" class="btn btn-bg-blue btn-sm btn-tooltip">
                                <span class="btn-text">Edit Circle Call</span>
                                <i class="bi bi-pen"></i>
                            </a>
                            <button class="btn btn-danger btn-sm btn-tooltip" onclick="deleteRow('{{ route('circlecall.delete', $circlecallData->id) }}')">
...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="card-title">IBM by Other</h4>
        </div>
        <hr class="mb-4">
        <!-- Table with stripped rows -->
        <div class="table-responsive">
            <table class="table table-striped table-hover mb-4">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $callWithData->member->firstName ?? '-' }} {{ $callWithData->member->lastName ?? '-' }}</td>
                        <td>{{ $callWithData->meetingPlace ?? '-' }}</td>
                        <td>
                            @if ($callWithData->meetingImage)
                            <img src="{{ url('meetingImage/' . basename($callWithData->meetingImage)) }}" alt="Meeting Image" style="width: 100px; height: auto; border-radius: 5px;">
                        @else
                            <span></span>
                        @endif
                        </td>
