...
<div class="tab-navigation mb-3" id="ibm-tabs">
    <a href="#" class="tab-btn {{ $activeTab == 'by_me' ? 'active' : '' }}" data-target="#tabByMe">By Me ({{
        $circlecall->total() }})</a>
    <a href="#" class="tab-btn {{ $activeTab == 'by_other' ? 'active' : '' }}" data-target="#tabByOther">By Other ({{
        $callWith->total() }})</a>
                    ...
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ optional($circlecallData->meetingPerson)->firstName ?? '-' }} {{
                                    optional($circlecallData->meetingPerson)->lastName ?? '-' }}</td>
                                <td>{{ $circlecallData->meetingPlace ?? '-' }}</td>
                                <td>
                                    @if ($circlecallData->meetingImage)
                                    <div class="meeting-image-wrapper" style="position: relative;">
                                        <img src="{{ url('meetingImage/' . basename($circlecallData->meetingImage)) }}"
                                ...
                                <td>{{ $circlecallData->date ?
                                    \Carbon\Carbon::parse($circlecallData->date)->format('d-m-Y') : '-' }}</td>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('circlecall.edit', $circlecallData->id) }}"
                                        class="btn btn-sm btn-bg-blue" title="Edit">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ optional($callWithData->member)->firstName ?? '-' }} {{
                                    optional($callWithData->member)->lastName ?? '-' }}</td>
                                <td>{{ $callWithData->meetingPlace ?? '-' }}</td>
                                <td>
                                    @if ($callWithData->meetingImage)
                                    <div class="meeting-image-wrapper" style="position: relative;">
                                        <img src="{{ url('meetingImage/' . basename($callWithData->meetingImage)) }}"
                                            alt="Meeting Image" style="width: 100px; height: auto; border-radius: 5px;"
                                            onclick="openImage(this)">
                                        <div class="meeting-image-overlay" onclick="closeImage(event)"></div>
                                    </div>
                                ...
                                <td>{{ $callWithData->date ? \Carbon\Carbon::parse($callWithData->date)->format('d-m-Y')
                                    : '-' }}</td>
                    ...
                    <div class="d-flex justify-content-end mt-3 custom-pagination">
                        {!! $callWith->appends(request()->query())->appends(['tab' =>
                        'by_other'])->fragment('ibm-tabs')->links() !!}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-lg-custom">
        <div class="modal-content border-0 rounded-3 shadow">
            ...
            <div class="modal-body">
                <form id="circlecallForm" enctype="multipart/form-data" method="post"
                    action="{{ route('circlecall.store') }}">
                    @csrf
                    ...
                    <div class="card p-3 shadow-sm border-0 rounded">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('circleId') is-invalid @enderror" id="circleId"
                                ...
                                <option value="" selected disabled>Select Circle</option>
                                @foreach ($circles as $circle)
                                <option value="{{ $circle->id }}" {{ old('circleId', auth()->user()->member->circleId)
                                    == $circle->id ? 'selected' : '' }}>
                                    {{ $circle->circleName }}
                                </option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('meetingPersonId') is-invalid @enderror"
                                id="meetingPersonName" placeholder="Select Member" readonly disabled
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="meetingPlace" name="meetingPlace"
                                placeholder="Enter place" required value="{{ old('meetingPlace') }}">
                        </div>

                        <!-- Meeting Image Upload -->
                        <div class="mb-3">
                            <label class="form-label fw-bold color-blue required">Upload Meeting Image <span
                            ...
                            <label for="meetingImage" class="upload-box @error('meetingImage') border-danger @enderror">
                                ...
                                <input type="file" class="file-input" id="meetingImage" name="meetingImage"
                            ...
                            <div class="mt-2">
                                <img id="photoPreview"
                                    src="{{ old('meetingImage') ? url('storage/' . old('meetingImage')) : asset('img/profile.png') }}"
                                    alt="Preview"
                        ...
                        <div class="mb-3">
                            ...
                            <?php
                                    ...
                                    ?>
                            <input type="date" class="form-control" id="date" name="date" min="{{ $pastLimit }}"
                                max="{{ $today }}" value="{{ $selectedDate }}" required>
                        </div>
                        @endif
                        ...
                        <!-- Remarks -->
                        <div class="mb-3">
                            <label for="remarks" class="form-label fw-bold color-blue">Remarks <span
                                    class="text-danger">*</span> </label>
