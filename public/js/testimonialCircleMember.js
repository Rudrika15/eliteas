            ...
            success: function (data) {
                if (data.length > 0) {
                    ...
                    data.forEach(function (member) {
                        ...
                        membersList += '<div id="' + memberId + '" class="card h-90 member-card" data-member-id="' + member.id + '">';
                    ...
                    $('.member-card').click(function () {
                        ...
                        $('#circlePersonId').val(memberId) ? $('#circlePersonId').val(memberId) : '';
                        $('#circlePersonName').val(memberName) ? $('#circlePersonName').val(memberName) : '';
                        $('#meetingPersonEmail').val(memberEmail) ? $('#meetingPersonEmail').val(memberEmail) : '';
                        $('#meetingPersonContact').val(memberContact) ? $('#meetingPersonContact').val(memberContact) : '';

                        // console.log("meetingPersonId", $('#meetingPersonName').val());
                        // console.log("meetingPersonName", memberName);

                        // Close the modal
                        $('#testimonialCircleMember').modal('hide');
                    });
                } else {
